...
class OnnxRnnTranslationsBase(OnnxTranslationBase):
    ...
    def extract_params_for_type(self, src_op, graph):
        ...
        log_assert(not self.params.clip,
                   "Node {}: SNPE does not support pre-activation clipping in RNN ops",
                   src_op.name)
        log_assert(not self.params.activation_alpha,
                   "Node {}: SNPE does not support activation alpha in RNN ops",
                   src_op.name)
        log_assert(not self.params.activation_beta,
                   "Node {}: SNPE does not support activation beta in RNN ops",
                   src_op.name)
        ...
        if self.params.direction == "bidirectional":
            self.params.activations = list(map(extract_activation, self.params.activations))
            ...
            self.params.activations = list(map(extract_activation, self.params.activations[0:self.no_of_gates]))
    ...
    def get_supported_version(self):
        return OP_VERSION_SUPPORTED[op_adapter.RnnTransformationOp.TRANSLATION_KEY]
...
class OnnxGruTranslation(OnnxRnnTranslationsBase):
    ...
    def extract_parameters(self, src_op, graph):
        self.params = extract_attributes(src_op,
                                         ('activation_alpha', 'lf', None),
                                         ('activation_beta', 'lf', None),
                                         ('activations', 'ls', ['Sigmoid', 'Sigmoid', 'Tanh']*2),
                                         ('clip', 'f', None),
                                         ('direction', 's', 'forward'),
                                         ('hidden_size', 'i'),
        ...
        # gru specific parameters and input checking
        log_assert(self.params.linear_before_reset == 0,
                   "Node {}: SNPE only supports linear_before_reset != 0 in GRU ops",
                   src_op.name)
    ...
    def get_supported_version(self):
        return OP_VERSION_SUPPORTED[op_adapter.GruOp.TRANSLATION_KEY]


OnnxTranslations.register_translation(OnnxGruTranslation(),
                          onnx_type('GRU'), op_adapter.GruOp.TRANSLATION_KEY)
...
class OnnxLSTMTranslation(OnnxRnnTranslationsBase):
    ...
    def extract_parameters(self, src_op, graph):
        default_activations = ['Sigmoid','Sigmoid', 'Sigmoid', 'Tanh'] * 2
        self.params = extract_attributes(src_op,
                                         ('activation_alpha', 'lf', None),
                                         ('activation_beta', 'lf', None),
                                         ('activations', 'ls', default_activations),
                                         ('clip', 'f', None),
                                         ('direction', 's', 'forward'),
                                         ('hidden_size', 'i'),
                                         ('input_forget', 'i', 0), )

        log_assert(self.params.input_forget == 0,
                   "Node {}: SNPE only supports linear_before_reset != 0 in LSTM ops",
                   src_op.name)
        log_assert(self.params.activations == default_activations,
                   "Node {}: SNPE does not support custom activations {} in LSTM ops",
                   src_op.name, str(self.params.activations))
    ...
    def get_supported_version(self):
        return OP_VERSION_SUPPORTED[op_adapter.LstmOp.TRANSLATION_KEY]


OnnxTranslations.register_translation(OnnxLSTMTranslation(),
                          onnx_type('LSTM'), op_adapter.LstmOp.TRANSLATION_KEY)
...
class OnnxRNNTranslation(OnnxRnnTranslationsBase):
        ...
        def extract_parameters(self, src_op, graph):
            ...
            log_assert(self.params.activations == default_activations,
                       "Node {}: SNPE does not support custom activations {} in RNN ops",
                       src_op.name, str(self.params.activations))
...
OnnxTranslations.register_translation(OnnxRNNTranslation(),
                          onnx_type('RNN'), op_adapter.RnnTransformationOp.TRANSLATION_KEY)
